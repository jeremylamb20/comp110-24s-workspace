...
def random_descending_list(n: int) -> list[int]:
    ...
    new_list: list[int] = []
    idx: int = 1
    new_list: list[int] = [randint(0, MAX_VAL)]
    while idx < n:
        new_list.append(new_list[idx-1] - randint(0, MAX_VAL))
        idx += 1
    return new_list
...
def evaluate_runtime(fn_name, start_size: int, end_size: int) -> np.array:
    ...
    for inp_size in range(start_size, end_size+1):
        ...
        call_funct: str = f"{fn_name}(l)"
        print(f"Trial {inp_size-start_size}/{end_size - start_size}")
        result = timeit.timeit(stmt=call_funct, globals=locals(), number=NUM_TRIALS)
